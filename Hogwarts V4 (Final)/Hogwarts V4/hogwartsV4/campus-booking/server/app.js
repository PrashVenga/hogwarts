// server/app.js — V4 server with V5 DB (clean drop-in)
// Robust date parsing, mutual block/booking protection, and DMY support.
...
/* Parse many date shapes → YYYY-MM-DD */
function toYMD(dLike) {
  if (!dLike) return null;
  const s = String(dLike).trim();
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;                     // YYYY-MM-DD
  let m = s.match(/^(\d{2})[/-](\d{2})[/-](\d{4})$/);              // DD-MM-YYYY / DD/MM/YYYY
  if (m) return `${m[3]}-${m[2]}-${m[1]}`;
  m = s.match(/^(\d{2})[/-](\d{2})[/-](\d{2})$/);                  // DD-MM-YY -> 20YY
  if (m) return `20${m[3]}-${m[2]}-${m[1]}`;
  m = s.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);                      // MM/DD/YYYY or DD/MM/YYYY
  if (m) {
    const a = Number(m[1]), b = Number(m[2]);
    if (a > 12 && b <= 12) return `${m[3]}-${m[2]}-${m[1]}`;       // DMY
    if (b > 12 && a <= 12) return `${m[3]}-${m[1]}-${m[2]}`;       // MDY
    return `${m[3]}-${m[2]}-${m[1]}`;                              // prefer DMY
  }
  const dt = new Date(s);
...
/* ---------------- utilities ---------------- */
// --- helpers to compose maintenance timestamps ---
const isTimeOnly = (v) => typeof v === 'string' && /^\d{2}:\d{2}(:\d{2})?$/.test(v);
const isDateOnly = (v) =>
  typeof v === 'string' && (
    /^\d{4}-\d{2}-\d{2}$/.test(v) ||              // YYYY-MM-DD
    /^\d{2}[/-]\d{2}[/-]\d{4}$/.test(v)           // DD-MM-YYYY or DD/MM/YYYY
  );

/** Build canonical "YYYY-MM-DDTHH:MM:SS" for start/end from body */
function buildBlockTimes(body) {
  let { date, start, end, start_time, end_time } = body || {};

  // Prefer UI trio when available
  let ymd = toYMD(date);

  // If date wasn’t provided, infer from any date-only start_time/end_time
  if (!ymd) {
...
app.get('/api/availability', async (req, res) => {
  ...
  try {
    ...
    // bookings for that day (return HH:MM-HH:MM to match UI)
    ...
    // GET /api/admin/breakdown?days=30  -> [{ facility, count }]
...
app.post('/api/book', async (req, res) => {
  ...
  try {
    ...
    // deny if overlaps any maintenance block (any block touching that calendar day)
...
/* create block — accepts:
   - start_time / end_time as ISO or DMY
   - OR date + start + end (HH:MM)
*/
app.post('/api/admin/blocks', adminOnly, async (req, res) => {
  try {
    ...
    // ✅ Compose start/end properly from request payload
